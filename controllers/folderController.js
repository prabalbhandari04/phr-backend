...
const getMyFolders = async(req,res) =>{
	try{
		const {id} = req.params;
		const folders = await Folder.find({user:id,parent:true});
		res.json(folders);
	}catch(err){
		return res.status(500).json({msg:err.message});
	}
...
const subFolders = async(req,res) =>{
	try {
		const {id} = req.params;
		const folders = await Folder.find({user:id,parent:false,parentid:req.params.id});
		const files = await File.find({user:id,folderid:req.params.id}).sort('-createdAt');
